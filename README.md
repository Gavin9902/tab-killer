...
| 🛡 **域名免杀** | 设置不想被自动关闭的域名白名单，Hover 面板一键切换 |
...
1. 下载源码并解压到某个固定目录（如 `~/tools/tab-killer`）
2. Chrome 打开 `chrome://extensions/`
3. 右上角开启「开发者模式」
4. 「加载已解压的扩展程序」→ 选择该目录

> **⚠️ 不要换目录安装**：扩展 ID 由目录路径决定，换目录会导致历史归档记录丢失。

### 更新到最新版

1. 下载最新源码
2. 用新文件**覆盖**原来那个目录（同名替换）
3. 打开 `chrome://extensions/`，找到 Tab Killer，点右下角 🔄 **刷新按钮**
4. 完成——所有归档记录和设置保持不变
...
在 Sidebar 给该页面写个备注，或把它钉在 Chrome 里——有音频播放、正在画中画的标签页也不会被归档。也可以把域名加入免杀白名单，新标签页 Hover 卡片点击眼睛图标即可。
</details>
